...
export default async function handler(
  ...
  response.setHeader(
    ...
    "X-CSRF-TOKEN, Content-Type, Authorization"
  );
  ...
  if (!service || !robloxPath) {
    return response
      .status(400)
      .json({ error: "Service and robloxPath are required query parameters." });
  }
  ...
  switch (service) {
    ...
    default:
      return response
        .status(400)
        .json({ error: `Unsupported Roblox API service: ${service}` });
  }
  ...
  const fullRobloxUrl = `${robloxApiBaseUrl}/${robloxPath}${
